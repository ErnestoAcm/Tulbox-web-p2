  ...
  const removeItem = (id) => {
    ...
    deleteData(`/api/my-list/${id}`).then((res) => {
      context.setAlertBox({
        open: true,
        error: false,
        msg: "Artículo eliminado de Mi lista!",
      });
      ...
      <section className="section cartPage">
        <div className="container">
          <div className="myListTableWrapper">
            <h2 className="hd mb-1">Mi lista</h2>
            <p>
              Hay <b className="text-red">{myListData?.length}</b> productos en
              tu Mi Lista
            </p>
            {myListData?.length !== 0 ? (
              <div className="row">
              ...
              <div className="empty d-flex align-items-center justify-content-center flex-column">
                ...
                <h3>Mi lista está vacía actualmente</h3>
                <br />
                <Link to="/">
                  {" "}
                  <Button className="btn-blue bg-red btn-lg btn-big btn-round">
                    <FaHome /> &nbsp; Seguir comprando
                  </Button>
                </Link>
              </div>
            )}
